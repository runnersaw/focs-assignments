...
No, there is no person that loves everybody else.

2. ∃y. ∀x. loves(x, y)

Yes, everybody loves c.

3. ∀x. ∃y. loves(x, y)

Yes, every person loves somebody else.

4. ∀y. ∃x. loves(x, y)

Yes, every person is loved by somebody.

5. ∃x. ∃y. loves(x, y)

Yes, somebody loves somebody.

6. ∃x. ∀y. ¬loves(x, y)

No, there is nobody that doesn't love anybody.

7. ∃x. ¬∀y. loves(x, y)

Yes, there is somebody that doesn't love everybody.
...
| ---------------------------------------- | ------- | ----- | ----- |
| ∀x, x. loves(x, x)                       |   No    |  Yes  |  Yes  |
| ∀x, y. loves(x, y) → loves(y, x)         |   No    |   No  |  Yes  |
| ∀x, y, z. loves(x, y) ∧ loves(y, z) → loves(x, z) |   No    |  Yes   |  Yes  |
...
sum(s(0), 0, s(0)) | 1
sum(s(0), s(0), s(s(0))) | prev, 2

2.  ∀x. sum( 0, x, x ) [hint:  induction]

sum(0,0,0) | 1
sum(0,s(0),s(0)) | prev, 2
sum(0,s(s(0)),s(s(0))) | prev, 2
sum(0,s(s(s(0))),s(s(s(0)))) | prev, 2
And so on...
...
No, it seems to be a greedy algorithm

c. Is `max_subarray` an example of dynamic programming? Why or why not?

No, you are not building up from a smaller problem.
...
No, it doesn't divide the array.

c. Is `binary_search_array` an example of dynamic programming? Why or why not?

Yes, it finds the solution of the larger function by finding the solution to the smaller function.
...
There isn't any advantage to memoization.
...
It provides no benefits because the function isn't called again with the same inputs. Memoization only benefits you when calling the function with the same inputs multiple times, such as when branching or needing the solution to the subproblem multiple times.
