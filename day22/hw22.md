...
Not really. The only one I think it might be is greedy, and I don't think it's really that because it isn't storing the best result so far.

b. If the text contains a word that is longer than `max_width`, what does this algorithm do?

Appends it to it's own line anyway without splitting it.

c. Does this algorithm produce an optimal result? (Do you have enough information to answer this question?)

Depends what "an optimal result is". For example, since it doesn't split a word longer than max length, it will produce some lines with more characters than max width.
...
This is definitely not recursive. Maybe you could argue that it's greedy given that it iterates through in the second part and finds the best break for that iteration, but in my mind this isn't greedy because it doesn't keep the best found answer to the problem yet over the whole loop. This might be dynamic programming (bottom up), because we're building a line_cost matrix that represents the costs to a bunch of subproblems (i.e. the cost of using words i-j in a line). However, I'm not sure if this should really be considered dynamic programming.
